...
uint8_t get_http_uri_name(uint8_t * uri, uint8_t * uri_buf)
{
	...
	if(!strcmp((char *)token ,"/IP")){
		int i = 0;
		while( token != NULL ) {

			if(i == 1){
				i = 0;
				api = (uint8_t *)strtok((char *)token, "&");
				while( api != NULL){


					if(i == 0){
						ip = memchr(api, '=', strlen(api));
						memmove(ip, ip+1, strlen(ip));

						printf( "ip : %s\n", ip );

						ip = (uint8_t *)strtok((char *)ip, ".");
						int j = 0;
						while( ip != NULL){
							gWIZNETINFO.ip[j] = atoi(ip);

							ip = strtok(NULL, ".");
							j++;
						}
					}else if(i == 1){
						sn = memchr(api, '=', strlen(api));
						memmove(sn, sn+1, strlen(sn));
						printf( "sn : %s\n", sn );

						sn = (uint8_t *)strtok((char *)sn, ".");
						int j = 0;
						while( sn != NULL){
							gWIZNETINFO.sn[j] = atoi(sn);
